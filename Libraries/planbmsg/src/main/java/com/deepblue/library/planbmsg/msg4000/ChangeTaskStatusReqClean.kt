...
class ChangeTaskStatusReqClean : Request(4002, "任务状态修改") {
    ...
    fun stop(reason: Int = 0, task_id: Int = 0): String {
        number = STOP
        json = Data("cancel", task_id, reason)
        return toString()
    }

    fun interrupt(reason: Int = 0, task_id: Int = 0): String {
        number = INTERRUPT
        json = Data("interrupt", task_id, reason)
        return toString()
    }
     ...
     * @param status 任务状态
     */
    class Data(val status: String, val task_id: Int? = null, val reason: Int? = 0)
}