package com.deepblue.deeplink.factory

import android.text.TextUtils
import java.util.regex.Pattern

object PhoneMatchUtil {
    /**
     * 中国电信号码格式验证 手机段： 133,149,153,173,177,180,181,189,199,1349,1410,1700,1701,1702
     */
    private const val CHINA_TELECOM_PATTERN =
        "(?:^(?:\\+86)?1(?:33|49|53|7[37]|8[019]|99)\\d{8}$)|(?:^(?:\\+86)?1349\\d{7}$)|(?:^(?:\\+86)?1410\\d{7}$)|(?:^(?:\\+86)?170[0-2]\\d{7}$)"
    /**
     * 中国联通号码格式验证 手机段：130,131,132,145,146,155,156,166,171,175,176,185,186,1704,1707,1708,1709
     */
    private const val CHINA_UNICOM_PATTERN =
     ...
     */
    private const val CHINA_MOBILE_PATTERN =
        "(?:^(?:\\+86)?1(?:3[4-9]|4[78]|5[0-27-9]|78|8[2-478]|98)\\d{8}$)|(?:^(?:\\+86)?1440\\d{7}$)|(?:^(?:\\+86)?170[356]\\d{7}$)"
     ...
     * @return
     */
    private fun checkChinaMobile(phone: String?): Boolean {
        if (!TextUtils.isEmpty(phone)) {
     ...
     * @return
     */
    private fun checkChinaUnicom(phone: String?): Boolean {
        if (!TextUtils.isEmpty(phone)) {
     ...
     * @return
     */
    private fun checkChinaTelecom(phone: String?): Boolean {
        if (!TextUtils.isEmpty(phone)) {
    ...
    fun hideMiddleMobile(phone: String): String {
        var phone = phone
        if (!TextUtils.isEmpty(phone)) {
            phone = phone.replace("(\\d{3})\\d{4}(\\d{4})".toRegex(), "$1****$2")
        }
        return phone
    }
}